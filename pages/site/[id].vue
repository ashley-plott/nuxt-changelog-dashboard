...
// site + maintenance
const { data, pending, error, refresh: refreshSite } = await useFetch(`/api/scheduler/sites/${id}`)
...
// ---- Tabs (added 'details')
const tab = ref<'calendar'|'changelog'|'notes'|'details'>('calendar')

// ---- Changelog
const selectedEnv = ref<string>('')
watchEffect(() => { if (site.value && !selectedEnv.value) selectedEnv.value = site.value.env || 'production' })
const clLimit = ref(20), clPkg = ref(''), clFrom = ref(''), clTo = ref('')
const headers = process.server ? useRequestHeaders(['cookie']) : undefined
const { data: clData, pending: clPending, refresh: clRefresh } = await useAsyncData(
  'site-changelogs',
  () => $fetch('/api/changelogs', {
    query: {
      site: id, env: selectedEnv.value || undefined, limit: clLimit.value,
...
function more() { clLimit.value += 20 }
...
async function saveDetails() {
  ...
  try {
    // Upsert site + ensure future maintenance entries
    ...
    await refreshSite()       // refresh site & maintenance
    detMsg.value = 'Saved.'
  } catch (e:any) {
    ...
    <div v-show="tab==='changelog'">
      ...
      <div class="rounded-2xl border p-5 bg-white shadow-sm space-y-4">
        <div class="flex flex-wrap items-end gap-3">
          ...
          <div><label class="block text-sm font-medium">Package</label><input v-model="clPkg" placeholder="vendor/package" class="border rounded px-3 py-2" /></div>
          <div class="flex items-end gap-2">
            <div><label class="block text-sm font-medium">From</label><input v-model="clFrom" type="datetime-local" class="border rounded px-3 py-2" /></div>
            <div><label class="block text-sm font-medium">To</label><input v-model="clTo" type="datetime-local" class="border rounded px-3 py-2" /></div>
          </div>
          <div><label class="block text-sm font-medium">Limit</label><input v-model.number="clLimit" type="number" min="1" class="border rounded px-3 py-2 w-24" /></div>
        ...
        <div v-else class="space-y-4">
          <div v-for="entry in clData?.items || []" :key="entry.run?.timestamp" class="rounded-xl border p-4">
            <div class="flex items-center justify-between">
              <div>
                <div class="font-medium">{{ new Date(entry.run?.timestamp).toLocaleString() }}</div>
                <div class="text-xs text-gray-500">
                  {{ entry.site?.env }}<span v-if="entry.run?.git_branch"> • {{ entry.run.git_branch }}</span><span v-if="entry.run?.git_sha"> ({{ entry.run.git_sha }})</span>
                </div>
              </div>
              <div class="flex gap-2">
            ...
            <div v-if="entry.summary?.has_changes" class="mt-3 overflow-auto">
              <table class="min-w-full text-sm">
                <thead><tr class="text-left border-b"><th class="py-2 pr-4">Package</th><th class="py-2 pr-4">Old</th><th class="py-2 pr-4">New</th></tr></thead>
                <tbody>
                  <tr v-for="p in entry.changes?.updated || []" :key="'u-'+p.name" class="border-b last:border-0"><td class="py-2 pr-4 font-medium">{{ p.name }}</td><td class="py-2 pr-4"><code>{{ p.old }}</code></td><td class="py-2 pr-4"><code>{{ p.new }}</code></td></tr>
                  <tr v-for="p in entry.changes?.added || []" :key="'a-'+p.name" class="border-b last:border-0"><td class="py-2 pr-4 font-medium text-emerald-700">{{ p.name }}</td><td class="py-2 pr-4"><em>—</em></td><td class="py-2 pr-4"><code>{{ p.new }}</code></td></tr>
                  <tr v-for="p in entry.changes?.removed || []" :key="'r-'+p.name" class="border-b last:border-0"><td class="py-2 pr-4 font-medium text-rose-700">{{ p.name }}</td><td class="py-2 pr-4"><code>{{ p.old }}</code></td><td class="py-2 pr-4"><em>—</em></td></tr>
                </tbody>
              </table>
            </div>
            <div v-else class="mt-3 text-sm text-gray-500">No dependency changes.</div>
            <div class="mt-2"><a v-if="entry.run?.ci_url" :href="entry.run.ci_url" target="_blank" class="text-blue-600 hover:underline text-sm">CI build</a></div>
          </div>
          <div class="flex justify-center"><button @click="more" class="px-4 py-2 rounded border">Load more</button></div>
        </div>
      </div>
    </div>
    ...
    <div v-show="tab==='notes'">
      ...
      <div v-if="authed" class="rounded-2xl border p-5 bg-white shadow-sm space-y-3">
        ...
        <div class="flex gap-3">
          <button @click="addNote" :disabled="saving" class="px-4 py-2 rounded bg-black text-white">{{ saving ? 'Saving…' : 'Add note' }}</button>
      ...
      <div class="mt-4 space-y-3">
        <div v-if="!notes?.items"><button @click="loadNotes" class="px-4 py-2 rounded border">Load notes</button></div>
        ...
        <div v-else v-for="n in notes.items" :key="n._id" class="rounded-xl border p-4 bg-white">
          <div class="flex items-start justify-between gap-3">
            ...
            <div v-if="canEdit(n)" class="shrink-0 flex gap-2">
      ...
      <div class="rounded-2xl border p-5 bg-white shadow-sm space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 mt-1">Pre-renewal maintenance is scheduled one month before this.</p>
          </div>
        </div>
        ...
        <p class="text-xs text-gray-500">
          Saving updates the site and ensures upcoming maintenance entries exist. If you want to fully regenerate past/future dates after changing the renew month, I can add a “rebuild schedule” endpoint.
        </p>
      </div>
    </div>
  </div>
